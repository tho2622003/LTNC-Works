...
bool init()
{
	...
	else
	{
		...
		//Create window
		gWindow = SDL_CreateWindow( "minesweeper by tho nguyn :v", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
		if( gWindow == NULL )
		{
...
bool loadMedia()
{
	...
	//Open the font
	gGameOver = TTF_OpenFont( "Font/COMIC.TTF", 30 );
	if( gGameOver == NULL )
	{
	...
	else
	{
		...
		if( !gTextTexture.loadFromRenderedText( "trash lmao", textColor ) )
		{
	...
	gPlayAgainWin = TTF_OpenFont( "Font/COMIC.TTF", 40 );
    ...
    gPlayAgainLose = TTF_OpenFont( "Font/COMIC.TTF", 40 );
...
void mineManager()
{
    ...
    if ( !gameOver && !isWinning )
    {
        ...
        //Erase the buffer
        mineLeft.str ( "" );
        mineLeft << "clear " << countMineLeft << " mines left";
